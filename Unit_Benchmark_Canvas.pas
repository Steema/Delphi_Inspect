...
type
  ...
  private
    ...
    procedure FinishChart(const ATitle,AFoot:String);
...
begin
  ...
  RunCanvasItem('Draw Rectangle x5',procedure begin TestCanvas.DrawRect(R,0,0,AllCorners,1); end,nil,0.2);
  ...
  RunCanvasItem('Radial Gradient x10',procedure begin
      TestCanvas.FillRect(R,0,0,AllCorners,1);
     end,
     procedure
     begin
  ...
  FinishChart('TCanvas Benchmark (FireMonkey '+
              FloatToStr(FireMonkeyVersion)+') '+TestCanvas.ClassName,
              RADIDE+' '+PlatformToString);
end;
{$ENDIF}
...
procedure TBenchmark.FinishChart(const ATitle,AFoot:String);
begin
  FChart.AddSeries(FSeries);
  FChart.Title.Caption:=ATitle;
  FChart.Foot.Caption:=AFoot;
